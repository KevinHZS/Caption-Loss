...
def eval_dci(model,image_processor,tokenizer,device,args):
    image_features = []
    text_features = []
    pred_true = 0
    ...
    with torch.no_grad():
        ...
        jsonfile_path = args.ann_file

        for json_n in os.listdir(jsonfile_path):
            jsonname = jsonfile_path+json_n

            with open(jsonname,"r",encoding='utf-8') as f:
                json_item = json.load(f)

            caption = json_item['extra_caption']
            lastname = json_item['image']


            image_name = image_path+lastname
            ...
            print(index,": ", len(os.listdir(jsonfile_path)))
        ...
        captionnums = len(os.listdir(jsonfile_path))
...
def eval_model(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained(args.model_base)
    device = model.device
    eval_dci(model,image_processor,tokenizer,device,args)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-folder", type=str, default="download_sam/138/")
    parser.add_argument("--image_size", type=int, default=224)
    parser.add_argument("--walk_type", type=str, default="long")
    parser.add_argument("--naflex", action='store_true', default=True)
    parser.add_argument("--ann_file", type=str, default="densely_captioned_images/annotations/")
